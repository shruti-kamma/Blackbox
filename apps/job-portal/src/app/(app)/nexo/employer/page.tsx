  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-4 py-12">
      <div className="mb-8 flex items-start justify-between gap-4">
        ...
        <Link href="/nexo/employer/jobs" className="whitespace-nowrap text-sm font-medium text-primary">
          Your postings →
        </Link>
      </div>
      ...
      {matches === null ? (
        ...
        <p className="text-muted-foreground">
          No matches yet.{" "}
          <Link href="/nexo/employer/jobs/new" className="font-medium text-primary">
            Post a role
          </Link>{" "}
          ...
          {jobGroups.map(([jobId, jobMatches]) => (
            <div key={jobId}>
              <div className="mb-2 flex items-center justify-between gap-4">
                ...
                <Link href={`/nexo/employer/jobs/${jobId}`} className="whitespace-nowrap text-sm font-medium text-primary">
                ...
                {jobMatches.slice(0, MATCHES_PREVIEW_PER_JOB).map((m) => (
                  <li key={m.id}>
                    <CandidateCard
                      ...
                      score={m.score}
                      profileHref={`/nexo/employer/candidates/${m.candidate.id}`}
                    />
