...
export default function EmployerCandidateProfilePage() {
  ...
  if (error) {
    return (
      <main className="mx-auto w-full max-w-3xl px-4 py-12">
        ...
        <Link href="/nexo/employer" className="text-sm font-medium text-primary">
  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-4 py-12">
      <Link href="/nexo/employer" className="mb-6 inline-block text-sm font-medium text-primary">
        ← Back to matched candidates
      </Link>
        ...
        <div className="mb-10">
          ...
          <ul className="flex flex-col gap-2">
            {data.applications.map((a) => (
              ...
              >
                <Link href={`/nexo/employer/jobs/${a.jobId}`} className="font-medium text-primary underline">
                  {a.jobTitle}
                </Link>
            ...
            {data.matches
              ...
              .map((m) => (
                <li
                  ...
                  className="flex items-center justify-between gap-4 rounded-md border border-border p-3 text-sm text-foreground"
                >
                  <Link href={`/nexo/employer/jobs/${m.jobId}`} className="font-medium text-primary underline">
                    {m.jobTitle}
                  </Link>
